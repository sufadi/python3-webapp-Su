...
class RequestHandler(object):
	...
	def __init__(self, app, fn):
		...
		logging.info("RequestHandler : %s->%s" % (app, fn))
		...
		if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
			if request.method == "POST":
				...
				if request.method == 'GET':
					...
					logging.info("request.method = get : %s" % request.query_string)
					if qs:
						kw = dict()
...
def add_route(app, fn):
	...
	logging.info('success add route %s %s => %s(%s)' % (method, path, fn.__name__, ', '.join(inspect.signature(fn).parameters.keys())))
